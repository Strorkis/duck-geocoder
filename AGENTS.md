...
| `uv` / `python` | **参考資料を読む** (PDFなど)。`mise exec -- uv run` 経由 | **配信データを作る工程** |
| `rclone` | R2へ上げる。**`copy` のみ** | **`sync`** (公開中のデータを消すため) |
...
- **`python` は参考資料を読むときだけ。** 線は「**読むのは調査、作るのはパイプライン**」。
  PDFのように他の道具で読めないものがあるので解禁したが、**配信データを作る工程には
  入れない。** 出力の再現性はRustに寄せてあり、依存も `cargo-audit` / `npm audit` で
  見ている。そこにロックの効かない面を増やさないため
  (`pyproject.toml` / `uv.lock` / `.python-version` で固定してはいる)
...
`sed -i` と**素の** `python` / `python3` / `uv` は `.claude/settings.json` のhookで止めている
(`mise exec --` 経由は通る)。**覚えている前提にすると取りこぼすため**
(このリポジトリでの実績: 1,052回のBashのうち `sed` 22回・`python3` 21回)。

## 約束の変え方

**ここの約束は固定ではない。** 守るコストが利益を上回るなら変えてよい。
**適材適所を、道具の禁止で潰さない。** 実際に3回変えている。

- `jq` は「持ち出さない」から「小さなJSONには使う」へ (hookが `jq` を要求したため)
- `sed` は「使わない」から「`-i` だけ止める」へ
  (パイプの中の置換は `awk` と変わらず、止める理由が無かった)
- `python` は「使わない」から「**参考資料を読むときだけ**」へ。
  参考資料のPDFが読めず、この環境には `pdftotext` も `pdftoppm` も無い。
  aptは使わない方針なので、**miseで `uv` を管理**して解いた。
  元の約束にあった「必要になったら mise に足して方針ごと見直す」をそのまま実行した形
...
- **メモリ不足によるテストの失敗。** アサーションの失敗に見えるが、原因はスワップ。
  E2Eは1 workerあたり3GB程度まで伸びる (Chromium + DuckDB-WASM) ので、
  **worker数はコア数ではなく搭載メモリから決めている** (`web/playwright.config.ts`)。
  Rust側の `cargo test` も `-j 4` を付けること (既定の16並列で7.9GBを食い尽くした実績)。
  落ちたら `free -h` と `/proc/pressure/memory` を先に見る
