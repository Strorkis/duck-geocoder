...
**ただし起動時の一箇所がまだ全ファイルを読む。** 用途の選択肢 (`fetchUsageOptions`) は
表示範囲で絞れない — 範囲外にしか無い用途を落とすと、その建物が絞り込みから消えるため。
ファイルが増えると**1ファイル1往復のフッター読み**が効いてくる。

**次にやること: 用途の語彙をカタログに入れる。** `catalog.json` は起動時にどのみち
読んでいる (11KB) ので、そこに列の取りうる値を書けば、起動時にparquetを1つも
読まずに済む。行政区域の名称を `overture_admin_names` として別ファイルに切り出したのと
同じ考え方を、カタログの中でやる形。
