...
**都市ごとに1ファイルのままでもUIは読める** (カタログの同じ `kind` を
`read_parquet([...])` で束ねる作りになっている)。ただし306ファイルになると
**ビュー作成時にフッターを306回読む**ので、いずれ1つに束ねたくなる。

**束ねる工程はまだ無い。** 素の `COPY` では `geo` メタデータ (WKBの宣言と
`covering.bbox`) が落ちるため、`overture_divisions_to_geoparquet` と同じように
統計を取って `KV_METADATA` を書き直す必要がある。何都市を配信するか決めてから作る。
